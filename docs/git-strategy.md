...
PR 머지 시 GitHub에서 원격 브랜치는 자동 삭제되지만, 로컬 브랜치는 그대로 남는다.
누적되면 `git branch`가 지저분해지므로 주기적으로 정리한다.
